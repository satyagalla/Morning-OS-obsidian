...
export class MorningOSSettingTab extends PluginSettingTab {
  ...
  clearDirty() {
    ...
    this.hideRunBanner();
    // update() only exists on Obsidian 1.13+ (declarative settings) — guard for older hosts.
    if (typeof this.update === "function") this.update();
  }

  // ── Declarative settings API (Obsidian 1.13+) ──────────────────────────
  // display() above remains the fallback for Obsidian < 1.13, which never
  // calls getSettingDefinitions(). Both must keep working.
  ...
  async setControlValue(key: string, value: unknown): Promise<void> {
    ...
    if (key === "intelligenceProvider") {
      const provider = value as string;
      this.plugin.settings.intelligenceModel = PROVIDER_DEFAULT_MODELS[provider] ?? "";
      this.plugin.settings.intelligenceRegion = provider === "bedrock" ? (this.plugin.settings.awsRegion || "us-east-2") : "";
    }
    if (key === "awsRegion") {
      this.plugin.settings.intelligenceRegion = value as string;
    }

    await this.plugin.saveData(this.plugin.settings);

    // Provider change reveals/hides different credential fields and changes the Model value shown.
    if (key === "intelligenceProvider") this.update();
  }

  getSettingDefinitions(): SettingDefinitionItem[] {
    return [
      { type: "page", name: "Briefing", items: this.briefingPageItems() },
      { type: "page", name: "AI", items: this.aiPageItems() },
      { type: "page", name: "Vault", items: this.vaultPageItems() },
      { type: "page", name: "Display", items: this.displayPageItems() },
      { type: "page", name: "Areas", page: () => createAreasSettingPage(this) },
      { type: "page", name: "About", items: [] },
    ];
  }

  private briefingPageItems(): SettingDefinitionItem[] {
    return [
      {
        type: "group",
        heading: "Briefing agent",
        items: [
          {
            name: "Carry lookback days",
            desc: "How many days to search back for the previous brief when detecting carried tasks.",
            control: { type: "number", key: "carryLookbackDays", min: 1, max: 30 },
          },
          {
            name: "Run agent now",
            desc: this.plugin.settings.settingsChangedSinceRun
              ? "⚠ Settings changed — run the agent to apply them."
              : "Manually trigger the briefing agent.",
            render: (setting) => {
          ...
          {
            name: "Refresh brief",
            desc: "Rebuild the brief from current vault files without calling the AI.",
            render: (setting) => {
              setting.addButton((btn) =>
  ...
  private aiPageItems(): SettingDefinitionItem[] {
    const passwordField = (name: string, key: "awsSecretAccessKey" | "openaiApiKey" | "geminiApiKey" | "groqApiKey", visible: () => boolean): SettingDefinition => ({
      name,
      visible,
      render: (setting) => {
    ...
    return [
      {
        ...
        items: [
          {
            name: "Provider",
            desc: "Which AI service generates your daily brief.",
            control: { type: "dropdown", key: "intelligenceProvider", options: PROVIDERS },
          },
          {
            name: "Model",
            desc: "Model ID to use. Defaults are pre-filled per provider.",
            control: { type: "text", key: "intelligenceModel" },
          },
          {
            name: "AWS Access Key ID",
            visible: () => this.plugin.settings.intelligenceProvider === "bedrock",
            control: { type: "text", key: "awsAccessKeyId" },
          },
          passwordField("AWS Secret Access Key", "awsSecretAccessKey", () => this.plugin.settings.intelligenceProvider === "bedrock"),
          {
            name: "AWS Region",
            desc: "e.g. us-east-2",
            visible: () => this.plugin.settings.intelligenceProvider === "bedrock",
            control: { type: "text", key: "awsRegion" },
          },
          passwordField("OpenAI API Key", "openaiApiKey", () => this.plugin.settings.intelligenceProvider === "openai"),
          passwordField("Gemini API Key", "geminiApiKey", () => this.plugin.settings.intelligenceProvider === "gemini"),
          passwordField("Groq API Key", "groqApiKey", () => this.plugin.settings.intelligenceProvider === "groq"),
        ],
      },
    ];
  }

  private vaultPageItems(): SettingDefinitionItem[] {
    ...
    return [
      {
        ...
        items: [
          {
            name: "Migrate vault to areas",
            desc: migrationDone
              ? "Migration complete. Your vault is using the new area system. Only re-run if you've added new content to old source files."
              : "Move your existing rules, goals, and tasks to the new area system. Old files are archived to _archive/pre-migration/. All paths below are relative to your vault root.",
            render: (setting) => {
              if (migrationDone) {
                setting.addButton((btn) => {
          ...
          },
          {
            ...
            desc: "Used by migration to archive old daily notes. Run migration to move these to _archive/daily-notes/ and remove this folder.",
            control: { type: "text", key: "dailyNoteDir", placeholder: "Essential/Daily" },
          },
          {
            name: "Briefs output folder",
            desc: "Where the agent writes daily brief JSON files.",
            control: { type: "text", key: "briefsDir", placeholder: "_generated/briefs" },
          },
          {
            name: "Feedback folder",
            desc: "Where the agent writes feedback and reaction files.",
            control: { type: "text", key: "feedbackDir", placeholder: "_generated/feedback" },
          },
          { name: "Tactical rules file", control: { type: "text", key: "sourceTacticalRules" } },
          { name: "Emotional rules file", control: { type: "text", key: "sourceEmotionalRules" } },
          { name: "Goals file", control: { type: "text", key: "sourceGoals" } },
          {
            name: "Technical tasks file",
            desc: "Legacy — tasks now live in the registry. Kept for brief context.",
            control: { type: "text", key: "sourceTechnicalTasks" },
          },
          {
            name: "Hobby tasks file",
            desc: "Legacy — tasks now live in the registry. Kept for brief context.",
            control: { type: "text", key: "sourceHobbyTasks" },
          },
          {
            name: "Identity Anchor file",
            desc: "5 static lines rendered at the top of the Home view.",
            control: { type: "text", key: "sourceIdentity" },
          },
          {
            name: "Wins log file",
            desc: "Chronological wins log. Plugin appends here when you add a win from the Home view.",
            control: { type: "text", key: "sourceWins" },
          },
        ],
      },
      {
        ...
        items: [
          {
            name: "Red alert heading",
            desc: "Daily note heading for urgent tasks. Must match exactly (case-insensitive).",
            control: { type: "text", key: "sectionRedAlert" },
          },
          { name: "Regular tasks heading", control: { type: "text", key: "sectionRegular" } },
          {
            name: "Wins heading",
            desc: "Used in both your daily note and the dashboard input.",
            control: { type: "text", key: "sectionWins" },
          },
          {
            name: "Thoughts heading",
            desc: "Daily note heading for unstructured captures.",
            control: { type: "text", key: "sectionThoughts" },
          },
          {
            name: "Top pending heading",
            desc: "Daily note heading for the top 3 pending items.",
            control: { type: "text", key: "sectionPending" },
          },
          {
            name: "Short-term goals heading",
            desc: "Heading in your goals file.",
            control: { type: "text", key: "goalsShortTerm" },
          },
          { name: "Long-term goals heading", control: { type: "text", key: "goalsLongTerm" } },
        ],
      },
    ];
  }

  private displayPageItems(): SettingDefinitionItem[] {
    const count = (name: string, key: NumericSettingsKey): SettingDefinition => ({
      name,
      control: { type: "number", key, min: 0 },
    });
    const toggle = (name: string, desc: string, key: keyof MorningOSSettings): SettingDefinition => ({
      name,
      desc,
    ...
    return [
      {
        type: "group",
        heading: "How many items to show",
        items: [
      ...
      },
      {
        ...
        items: [
          toggle(
            "Require subtasks complete before parent",
            "When on, a task with open subtasks can't be checked off until every subtask is done. When off, a task's own checkbox is independent of its subtasks.",
            "requireSubtasksComplete"
          ),
        ],
      },
      {
        ...
        items: [
          toggle(
            ...
            "When on, tasks with notes show a small indicator on their row.",
            "showNotesIndicator"
          ),
        ],
      },
  ...
  renderAreasSection(containerEl: HTMLElement) {
...
// The areas builder is a stateful two-panel editor, not a flat list of
// settings — it stays fully imperative via renderAreasSection() and is
// wrapped as a declarative sub-page's `page` factory (the documented escape
// hatch for content that "doesn't fit cleanly into a list of definitions").
//
// SettingPage only exists on Obsidian 1.13+. Defining the subclass inside a
// function (rather than at module scope) defers evaluating `extends
// SettingPage` until this factory actually runs — which only happens when
// the 1.13+ runtime calls getSettingDefinitions()'s `page` callback. On
// older Obsidian, which never calls that, this function is never invoked,
// so SettingPage being undefined there never matters.
function createAreasSettingPage(tab: MorningOSSettingTab): SettingPage {
  class MorningOSAreasPage extends SettingPage {
    constructor() {
      super();
      this.title = "Areas";
    }
    display(): void {
      ...
      tab.renderAreasSection(this.containerEl);
    }
  }
  return new MorningOSAreasPage();
}
